...
namespace codestellar.bolierplate.Tests
{
    public abstract class bolierplateTestBase : AbpIntegratedTestBase<bolierplateTestModule>
    {
        ...
        protected void UsingDbContext(Action<bolierplateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<bolierplateDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<bolierplateDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<bolierplateDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<bolierplateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<bolierplateDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<bolierplateDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<bolierplateDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<bolierplateDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<bolierplateDbContext, Task<T>> func)
        {
            T result;
